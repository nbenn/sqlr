...
test_that("columns can be listed", {
  ...
  expect_named(show_db_cols("plane"), c(
    "Field", "Type", "Null", "Key",
    "Default", "Extra"
  ))
  ...
  expect_equal(
    nrow(show_db_cols("plane", where = DBI::SQL("Field = 'year'"))),
    1L
  )
  expect_equal(ncol(show_db_cols("plane", full = TRUE)), 9L)
  expect_named(
    show_db_cols("plane", full = TRUE),
    c(
  ...
  expect_named(
    show_db_cols("plane", parse = TRUE),
    c(
      "Field", "Type", "Length", "Unsigned", "Null", "Key",
      "Default", "Extra"
    )
  )
  expect_equal(ncol(show_db_cols("plane", full = TRUE, parse = TRUE)), 12L)
  expect_named(
    show_db_cols("plane", full = TRUE, parse = TRUE),
    c(
      "Field", "Type", "Length", "Unsigned", "Charset",
...
test_that("mysql identifiers can be unqouted", {
  expect_equal(
    unquote_ident(DBI::dbQuoteIdentifier(sqlr:::get_con(), "foo")),
    "foo"
  )
  expect_equal(
    unquote_ident(DBI::dbQuoteIdentifier(
      sqlr:::get_con(),
      c("foo", "bar")
    )),
    c("foo", "bar")
  ...
  expect_equal(
    unquote_ident(DBI::dbQuoteIdentifier(sqlr:::get_con(), "fo'o")),
    "fo'o"
  )
  expect_equal(
    unquote_ident(DBI::dbQuoteIdentifier(sqlr:::get_con(), "fo`o")),
...
test_that("mysql strings can be unqouted", {
  expect_equal(
    unquote_str(DBI::dbQuoteString(sqlr:::get_con(), "foo")),
    "foo"
  )
  expect_equal(
    unquote_str(DBI::dbQuoteString(
      sqlr:::get_con(),
      c("foo", "bar")
    )),
    c("foo", "bar")
  ...
  expect_equal(
    unquote_str(DBI::dbQuoteString(sqlr:::get_con(), "fo'o")),
    "fo'o"
  )
  expect_equal(
    unquote_str(DBI::dbQuoteString(sqlr:::get_con(), "fo''o")),
  ...
  expect_equal(
    unquote_str(DBI::dbQuoteString(sqlr:::get_con(), "fo\no")),
    "fo\no"
  )
  expect_equal(
    unquote_str(DBI::dbQuoteString(sqlr:::get_con(), "fo\ro")),
    "fo\ro"
  )
  expect_equal(
    unquote_str(DBI::dbQuoteString(sqlr:::get_con(), "fo\to")),
  ...
  expect_equal(
    unquote_str(DBI::dbQuoteString(sqlr:::get_con(), "fo\x1ao")),
    "fo\x1ao"
  )
  expect_equal(
    unquote_str(DBI::dbQuoteString(sqlr:::get_con(), "fo\032o")),
    "fo\032o"
  )
  expect_equal(
    unquote_str(DBI::dbQuoteString(sqlr:::get_con(), "fo-$!o")),
  ...
  expect_equal(
    unquote_str(DBI::dbQuoteString(sqlr:::get_con(), "öääü")),
    "öääü"
  )
  expect_equal(
    unquote_str(DBI::dbQuoteString(sqlr:::get_con(), "@ª]|{")),
    "@ª]|{"
  )
})
...
test_that("column definitions can be parsed", {
  ...
  expect_equal(
    ...
    tibble::tibble(
      Field = "foo", Type = c("INT", "DOUBLE", "INT"),
