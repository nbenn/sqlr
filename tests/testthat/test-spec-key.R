...
test_that("references can be specified", {
  ...
  expect_s4_class(fk_spec("foo", "bar", "foobar", check = FALSE),
                  "SQL")
...
test_that("primary keys can be specified", {
  ...
  expect_s4_class(pk_spec("foo"),
                  "SQL")
  expect_equal(as.character(pk_spec("foo")),
               "PRIMARY KEY (`foo`)")
  expect_equal(as.character(pk_spec(c("foo", "bar"))),
               "PRIMARY KEY (`foo`, `bar`)")
  expect_equal(as.character(pk_spec("fo'o")),
               "PRIMARY KEY (`fo'o`)")
  expect_equal(as.character(pk_spec("b\nar")),
               "PRIMARY KEY (`b\nar`)")
  expect_equal(as.character(pk_spec("foo`bar")),
               "PRIMARY KEY (`foo``bar`)")
  expect_equal(as.character(pk_spec("foo", type = "btree")),
               "PRIMARY KEY USING BTREE (`foo`)")
  expect_equal(as.character(pk_spec("foo", constr_name = "bar")),
               "CONSTRAINT `bar` PRIMARY KEY (`foo`)")
  expect_equal(as.character(pk_spec("foo", constr_name = "b`ar")),
               "CONSTRAINT `b``ar` PRIMARY KEY (`foo`)")
  expect_equal(as.character(pk_spec("foo", block_size = 4L)),
               "PRIMARY KEY (`foo`) KEY_BLOCK_SIZE = 4")
  expect_equal(as.character(pk_spec("foo", comment = "foobar")),
               "PRIMARY KEY (`foo`) COMMENT 'foobar'")
  expect_equal(as.character(pk_spec("foo", comment = "fo`obar")),
...
test_that("unique keys can be specified", {
  ...
  expect_s4_class(uk_spec("foo"),
                  "SQL")
  expect_equal(as.character(uk_spec("foo")),
               "UNIQUE KEY (`foo`)")
  expect_equal(as.character(uk_spec(c("foo", "bar"))),
               "UNIQUE KEY (`foo`, `bar`)")
  expect_equal(as.character(uk_spec("fo'o")),
               "UNIQUE KEY (`fo'o`)")
  expect_equal(as.character(uk_spec("b\nar")),
               "UNIQUE KEY (`b\nar`)")
  expect_equal(as.character(uk_spec("foo`bar")),
               "UNIQUE KEY (`foo``bar`)")
  expect_equal(as.character(uk_spec("foo", type = "btree")),
               "UNIQUE KEY USING BTREE (`foo`)")
  expect_equal(as.character(uk_spec("foo", constr_name = "bar")),
               "CONSTRAINT `bar` UNIQUE KEY (`foo`)")
  expect_equal(as.character(uk_spec("foo", constr_name = "b`ar")),
               "CONSTRAINT `b``ar` UNIQUE KEY (`foo`)")
  expect_equal(as.character(uk_spec("foo", block_size = 4L)),
               "UNIQUE KEY (`foo`) KEY_BLOCK_SIZE = 4")
  expect_equal(as.character(uk_spec("foo", comment = "foobar")),
               "UNIQUE KEY (`foo`) COMMENT 'foobar'")
  expect_equal(as.character(uk_spec("foo", comment = "fo`obar")),
               "UNIQUE KEY (`foo`) COMMENT 'fo`obar'")
  expect_equal(as.character(uk_spec("foo", comment = "fo\nobar")),
               "UNIQUE KEY (`foo`) COMMENT 'fo\\nobar'")
  expect_equal(as.character(uk_spec("foo", comment = "fo'obar")),
               "UNIQUE KEY (`foo`) COMMENT 'fo\\'obar'")
  expect_equal(as.character(uk_spec("foo", "bar", "baz")),
...
test_that("secondary keys can be specified", {
  ...
  expect_s4_class(key_spec("foo"),
                  "SQL")
  expect_equal(as.character(key_spec("foo")),
               "KEY (`foo`)")
  expect_equal(as.character(key_spec(c("foo", "bar"))),
               "KEY (`foo`, `bar`)")
  expect_equal(as.character(key_spec("fo'o")),
               "KEY (`fo'o`)")
  expect_equal(as.character(key_spec("b\nar")),
               "KEY (`b\nar`)")
  expect_equal(as.character(key_spec("foo`bar")),
               "KEY (`foo``bar`)")
  expect_equal(as.character(key_spec("foo", type = "btree")),
               "KEY USING BTREE (`foo`)")
  expect_equal(as.character(key_spec("foo", block_size = 4L)),
               "KEY (`foo`) KEY_BLOCK_SIZE = 4")
  expect_equal(as.character(key_spec("foo", comment = "foobar")),
               "KEY (`foo`) COMMENT 'foobar'")
  expect_equal(as.character(key_spec("foo", comment = "fo`obar")),
...
test_that("fulltext keys can be specified", {
  ...
  expect_s4_class(ft_key_spec("foo"),
                  "SQL")
  expect_equal(as.character(ft_key_spec("foo")),
               "FULLTEXT KEY (`foo`)")
  expect_equal(as.character(ft_key_spec(c("foo", "bar"))),
               "FULLTEXT KEY (`foo`, `bar`)")
  expect_equal(as.character(ft_key_spec("fo'o")),
               "FULLTEXT KEY (`fo'o`)")
  expect_equal(as.character(ft_key_spec("b\nar")),
               "FULLTEXT KEY (`b\nar`)")
  expect_equal(as.character(ft_key_spec("foo`bar")),
               "FULLTEXT KEY (`foo``bar`)")
  expect_equal(as.character(ft_key_spec("foo", parser = "bar")),
...
test_that("spatial keys can be specified", {
  ...
  expect_s4_class(spat_key_spec("foo"),
                  "SQL")
  expect_equal(as.character(spat_key_spec("foo")),
               "SPATIAL KEY (`foo`)")
  expect_equal(as.character(spat_key_spec(c("foo", "bar"))),
               "SPATIAL KEY (`foo`, `bar`)")
  expect_equal(as.character(spat_key_spec("fo'o")),
               "SPATIAL KEY (`fo'o`)")
  expect_equal(as.character(spat_key_spec("b\nar")),
               "SPATIAL KEY (`b\nar`)")
  expect_equal(as.character(spat_key_spec("foo`bar")),
               "SPATIAL KEY (`foo``bar`)")
