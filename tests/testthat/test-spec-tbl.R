...
test_that("tables can be specified", {
  ...
  expect_equal(as.character(tbl_spec("foo", auto_incr = 5L)),
               paste("CREATE TABLE `foo` (`id` INT UNSIGNED NOT NULL",
                     "AUTO_INCREMENT PRIMARY KEY) ENGINE = 'InnoDB'",
                     "AUTO_INCREMENT = 5"))
  expect_equal(as.character(tbl_spec("foo", char_set = "foobar")),
               paste("CREATE TABLE `foo` (`id` INT UNSIGNED NOT NULL",
                     "AUTO_INCREMENT PRIMARY KEY) ENGINE = 'InnoDB'",
